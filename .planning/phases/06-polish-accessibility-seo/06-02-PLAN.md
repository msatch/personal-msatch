...
must_haves:
  truths:
    ...
    - "Mobile LCP is under 2.5 seconds with minimal JS bundle"
  artifacts:
...
<task type="auto">
  ...
  <verify>
    ...
    2. Check build output for page sizes -- note First Load JS for each route
  ...
  <done>
    Scroll reveal animations appear on below-the-fold sections across home, bio, and services pages. CTA buttons have enhanced hover (scale + shadow) and active (scale-down) states. Service cards lift on hover. All animations respect prefers-reduced-motion. Build succeeds and First Load JS remains minimal.
  </done>
</task>
...
7. Build output shows reasonable JS bundle sizes (under ~100 KB First Load JS per page)
</verification>
...
- Performance: `npm run build` shows reasonable bundle, no page exceeds 100 KB First Load JS
</success_criteria>
